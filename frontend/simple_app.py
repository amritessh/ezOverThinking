...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown(
            f'<div class="anxiety-indicator {anxiety_class}">{st.session_state["anxiety_level"].upper()}</div>',
            unsafe_allow_html=True,
        )
    ...
    for message in st.session_state["messages"]:
        if message["role"] == "user":
            ...
            st.markdown(
                f'<div class="chat-message agent-message"><strong>{message["agent"]}:</strong> {message["content"]}</div>',
                unsafe_allow_html=True,
            )
                ...
                if user_input.strip():
                    ...
                    with st.spinner("🤔 The agents are overthinking..."):
                        try:
                            ...
                            agent_content = (
                                f"Sorry, could not connect to backend. Error: {str(e)}"
                            )
                            agent_name = "System"
                            escalation = 0
