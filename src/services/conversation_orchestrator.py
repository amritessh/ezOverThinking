...
from ..agents.coordinator import AgentCoordinator, CoordinationStrategy, ConversationPhase
from ..agents.communication_protocol import communication_protocol, MessageType, HandoffRequest
...
class ConversationOrchestrator:
    ...
    async def start_conversation(
        ...
        try:
            ...
            await self.state_manager.store_conversation_state(conversation_id, conversation_state)
    ...
    async def orchestrate_response(
        ...
        try:
            ...
            elif conversation_context.orchestration_mode == OrchestrationMode.COLLABORATIVE:
            ...
            if self.anxiety_tracker:
                await self.anxiety_tracker.track_anxiety_change(
                    ...
                    AnxietyLevel(min(5, conversation_state.current_anxiety_level.value + response.anxiety_escalation - 1))
                )
    ...
    async def _orchestrate_collaborative(
        ...
        for agent in selected_agents:
            try:
                ...
                self.logger.error(f"Error getting response from agent {agent.name}: {e}")
    ...
    async def _orchestrate_adaptive(
        ...
        if analysis["should_change_strategy"]:
            ...
            await self.state_manager.update_conversation_state(
                conversation_id,
                {"context": {**conversation_state.context, "strategy": new_strategy.value}}
            )
    ...
    async def _select_collaborative_agents(
        ...
        primary_agent = await self._select_simple_agent(user_message, conversation_state)
        if primary_agent:
    ...
    async def _synthesize_collaborative_response(
        ...
        combined_content = "Here's what multiple experts are saying about your situation:\n\n"
    ...
    async def _analyze_conversation_patterns(
        ...
        if conversation_state.message_count > 10:
            ...
            if conversation_state.escalation_count < 3:
                ...
                analysis["recommended_strategy"] = CoordinationStrategy.SPIRAL_INTENSIFICATION
        ...
        if len(conversation_state.agents_involved) > 4:
            ...
            analysis["should_change_strategy"] = True
            analysis["recommended_strategy"] = CoordinationStrategy.COLLABORATIVE_DESTRUCTION
    ...
    async def _select_simple_agent(
        ...
        # Social keywords
        if any(word in message_lower for word in ["social", "people", "friends", "embarrassed"]):
            return self._get_agent_by_type(AgentType.SOCIAL_ANXIETY_AMPLIFIER)
        
        # Time/urgency keywords
        if any(word in message_lower for word in ["time", "deadline", "urgent", "running out"]):
            return self._get_agent_by_type(AgentType.TIMELINE_PANIC_GENERATOR)
        
        # Statistics/probability keywords
        if any(word in message_lower for word in ["chance", "likely", "probably", "statistics"]):
    ...
    async def _update_conversation_state(
        ...
        updates = {
            ...
            "current_anxiety_level": AnxietyLevel(
                min(5, conversation_state.current_anxiety_level.value + response.anxiety_escalation - 1)
            ),
        ...
        if response.agent_id not in conversation_state.agents_involved:
            updates["agents_involved"] = conversation_state.agents_involved + [response.agent_id]
    ...
    async def _record_orchestration_event(
        ...
        # Update metrics
        agent_metrics = self.orchestration_metrics["agent_utilization"].get(response.agent_id, {})
        agent_metrics["total_interactions"] = agent_metrics.get("total_interactions", 0) + 1
        
        self.orchestration_metrics["agent_utilization"][response.agent_id] = agent_metrics
    ...
    async def _record_conversation_completion(
        self, 
        conversation_id: str, 
        reason: str
    ):
        ...
        try:
            ...
            self.orchestration_metrics["average_conversation_length"] = (
                (current_avg * (total_convs - 1) + conversation_state.message_count) / total_convs
            )
    ...
    async def get_conversation_analytics(
        ...
        try:
            ...
            analytics = {
                ...
                "duration_seconds": (datetime.now() - conversation_state.created_at).total_seconds(),
                "total_messages": conversation_state.message_count,
