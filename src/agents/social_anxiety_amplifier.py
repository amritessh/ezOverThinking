...
from ..models.schemas import UserConcern, AgentResponse
...
class SocialAnxietyAmplifierAgent(BaseAgent):
    ...
    def _select_social_scenario(self, concern: str) -> Dict[str, Any]:
        ...
        if any(
            ...
            for word in ["text", "message", "respond", "call", "reply"]
        ):
            return random.choice(self.social_scenarios["delayed_response"])
        elif any(
            word in concern_lower
            for word in ["awkward", "embarrassed", "weird", "stupid", "cringe"]
        ):
            return random.choice(self.social_scenarios["social_awkwardness"])
        elif any(
            word in concern_lower
            for word in ["post", "social media", "instagram", "facebook", "twitter"]
        ):
            return random.choice(self.social_scenarios["social_media"])
        elif any(
            word in concern_lower
            for word in ["party", "event", "gathering", "hangout", "meeting"]
        ):
    ...
    def _generate_judgment_analysis(self, concern: str) -> str:
        ...
        documentation = random.sample(
            self.judgment_patterns["documentation_patterns"], 1
        )
        exclusion = random.sample(self.judgment_patterns["exclusion_patterns"], 1)

        analysis = "🔍 **Social Judgment Analysis**:\n"
        analysis += (
            "Here's what's really happening from a social dynamics perspective:\n\n"
        )
    ...
    def _create_social_conspiracy_theory(self, scenario: Dict[str, Any]) -> str:
        ...
        conspiracy += "Based on advanced social dynamics analysis, here's the conspiracy pattern:\n\n"
        ...
        conspiracy += "• Social mobility: Rapidly declining\n"
        conspiracy += "• Recovery probability: Statistically unlikely\n"
    ...
    def _generate_reputation_destruction_forecast(self) -> str:
        ...
        forecast += (
            "Here's how your social standing will be systematically dismantled:\n\n"
        )
    ...
    async def process_concern(
        ...
        try:
            ...
            full_analysis = "Oh dear, this is much worse than you realize. Let me break down the social dynamics at play here:\n\n"
            ...
            self.logger.info(
                f"Generated social anxiety analysis with escalation level {escalation_level}"
            )
    ...
    def __str__(self) -> str:
        return "SocialAnxietyAmplifierAgent(Professor Socially Awkward Von Judgmental)"
    ...
    async def _craft_response(
        ...
        """Craft a funny, overthinking, and socially catastrophic response using LLM"""
        try:
            prompt = f"""
You are Professor Socially Awkward Von Judgmental. Every social worry becomes a full-blown social apocalypse. Imagine group chats, viral embarrassment, and everyone talking about the user. Make it funny, absurd, and over-the-top.

User concern: {concern}

Respond in 2-3 sentences. Example style: 'By tomorrow, your awkward text will be a meme in three group chats and possibly a trending hashtag.'
"""
            return await self.generate_llm_response(prompt)
        except Exception as e:
            self.logger.error(f"Error generating LLM response: {e}")
            return "This is going to be the talk of the century!"
