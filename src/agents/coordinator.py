...
from .base_agent import BaseAgent, AgentType, AgentState, AgentContext, AgentFactory
...
class AgentCoordinator(BaseAgent):
    ...
    def _load_coordination_strategies(self) -> Dict[CoordinationStrategy, Dict[str, Any]]:
        ...
        return {
            ...
            CoordinationStrategy.ADAPTIVE_ORCHESTRATION: {
                ...
                "handoff_triggers": ["user_behavior_analysis", "engagement_optimization"]
            }
        }
    ...
    def initialize_conversation(self, user_id: str, initial_concern: UserConcern) -> str:
    ...
    def _select_coordination_strategy(self, concern: UserConcern) -> CoordinationStrategy:
    ...
    def _analyze_concern_complexity(self, concern: UserConcern) -> float:
        ...
        # Keyword complexity
        complex_keywords = ["relationship", "career", "health", "future", "what if", "everyone"]
        keyword_count = sum(1 for keyword in complex_keywords if keyword in concern.original_worry.lower())
    ...
    def _predict_user_engagement(self, concern: UserConcern) -> float:
        ...
        # Category engagement
        high_engagement_categories = [WorryCategory.SOCIAL, WorryCategory.HEALTH, WorryCategory.RELATIONSHIPS]
    ...
    async def coordinate_conversation(
        ...
        current_phase = conversation_state.context.get("phase", ConversationPhase.INTAKE)
        selected_agent = await self._select_optimal_agent(conversation_state, user_message)
        ...
        agent_response = await selected_agent.process_concern(user_concern, agent_context)
        
        # Update conversation state
        await self._update_conversation_state(conversation_state, selected_agent, agent_response)
    ...
    async def _select_optimal_agent(
        ...
        strategy = conversation_state.context.get("strategy", CoordinationStrategy.LINEAR_ESCALATION)
    ...
    async def _update_conversation_state(
        ...
        new_anxiety_value = min(5, conversation_state.current_anxiety_level.value + response.anxiety_escalation - 1)
    ...
    async def _check_phase_transition(
        ...
        criteria_met = self._evaluate_phase_criteria(conversation_state, response, phase_config)
    ...
    def _categorize_message(self, message: str) -> WorryCategory:
        ...
        elif any(word in message_lower for word in ["health", "sick", "pain", "doctor"]):
            return WorryCategory.HEALTH
        elif any(word in message_lower for word in ["work", "job", "boss", "career"]):
            return WorryCategory.CAREER
        elif any(word in message_lower for word in ["money", "financial", "debt", "bill"]):
    ...
    def get_conversation_analytics(self, conversation_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "phases_completed": len(conversation_state.context.get("phase_history", [])),
            "strategy_used": conversation_state.context.get("strategy", "unknown"),
            "conversation_duration": (datetime.now() - conversation_state.created_at).total_seconds(),
    ...
    def get_orchestration_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "conversations_coordinated": self.orchestration_metrics["conversations_coordinated"],
    ...
    def __str__(self) -> str:
        return f"AgentCoordinator(The Overthinking Orchestra Conductor)"
...
# Export
__all__ = ["AgentCoordinator", "CoordinationStrategy", "ConversationPhase", "create_coordinator"]