...
class ProbabilityTwisterAgent(BaseAgent):
    ...
    def _create_fake_study_citation(self, category: WorryCategory) -> Dict[str, Any]:
        ...
        if not studies:
            return {
                ...
                "findings": [
                    "Statistical analysis confirms escalating probability patterns"
                ],
    ...
    def _generate_statistical_analysis(
        ...
        analysis += "mathematical relationship between your current concerns and probable outcomes.\n\n"
        ...
        analysis += "that your concerns will manifest in measurable ways. "
        analysis += f"The data suggests you're {probability_multiplier}x more likely to experience "
        analysis += "the outcomes you're worried about.\n\n"
    ...
    def _create_false_correlation(self, concern: str, category: WorryCategory) -> str:
        ...
        correlation_text = "🔗 **Correlation Analysis**: "
        correlation_text += (
            f"There's a {correlation_coefficient} correlation between {correlation}. "
        )
        correlation_text += (
            "This means that your level of concern is mathematically predictive "
        )
        correlation_text += "of the actual probability of the outcome occurring."

        return correlation_text

    def _suggest_next_agent(
        self, category: WorryCategory, probability_level: float
    ) -> str:
    ...
    async def process_concern(
        ...
        try:
            ...
            statistical_analysis = self._generate_statistical_analysis(
                concern.original_worry, category
            )

            # Create false correlation
            correlation_analysis = self._create_false_correlation(
                concern.original_worry, category
            )
            ...
            self.logger.info(
                f"Generated statistical analysis for {category.value} concern with {probability_level} correlation"
            )
    ...
    async def _craft_response(
        ...
        """Craft a funny, overthinking, and statistical response using LLM"""
        try:
            prompt = f"""
...
"""
            ...
            return "Statistically, this is a 99% chance of spiraling!"

    def __str__(self) -> str:
        return "ProbabilityTwisterAgent(Dr. Probability McStatistics)"
