...
class FalseComfortProviderAgent(BaseAgent):
    ...
    def _generate_comfort_undermining_sequence(
        self, concern: str, category: WorryCategory
    ) -> str:
        ...
        # Get exception types
        probability_exception = random.choice(
            self.reassurance_exceptions["probability_exceptions"]
        )
        timing_exception = random.choice(
            self.reassurance_exceptions["timing_exceptions"]
        )
        hidden_variable_exception = random.choice(
            self.reassurance_exceptions["hidden_variable_exceptions"]
        )

        # Build sequence
        sequence = "💙 **Comfort & Reassurance**:\n"
        ...
        sequence += "🤗 **Initial Reassurance**:\n"
        sequence += "In most cases, situations like this resolve themselves naturally. You're dealing with something that thousands of people face every day, and the vast majority of them come out just fine. There's really no need to panic.\n\n"

        sequence += "😟 **However... Some Concerns**:\n"
        ...
        sequence += "⚠️ **The Uncomfortable Truth**:\n"
    ...
    def _generate_false_hope_destruction(self) -> str:
        ...
        hope_builder = random.choice(
            self.hope_destruction_techniques["false_hope_builders"]
        )
        hope_destroyer = random.choice(
            self.hope_destruction_techniques["hope_destroyers"]
        )
        devastating_follow_up = random.choice(
            self.hope_destruction_techniques["devastating_follow_ups"]
        )

        destruction = "🌟 **Reasons for Hope**:\n"
        ...
        destruction += "💔 **But Hope Can Be Dangerous**:\n"
        destruction += f"{hope_destroyer} I care about you too much to let you be blindsided by false optimism. {devastating_follow_up}\n\n"

        destruction += "🎭 **The Caring Truth**:\n"
        destruction += "I'm telling you this because I care about you, not because I want to hurt you. Sometimes the most loving thing is to help someone prepare for difficult possibilities. I'd rather you be prepared and pleasantly surprised than hopeful and devastated."
    ...
    def _create_what_if_scenarios(self, concern: str, category: WorryCategory) -> str:
        ...
        scenarios = "🤔 **What If Scenarios** (I hate to mention these, but...):\n"
        scenarios += "As much as I want to be purely reassuring, I care about you too much to leave you unprepared:\n\n"

        if category == WorryCategory.SOCIAL:
            scenarios += (
                "• What if the people involved are more observant than you think?\n"
            )
            scenarios += "• What if this situation is being discussed more widely than you realize?\n"
            scenarios += "• What if your social intuition is actually picking up on real signals?\n"
            scenarios += (
                "• What if the timing makes this more significant than usual?\n"
            )
            scenarios += "• What if this is the situation that people remember and talk about?\n"

        elif category == WorryCategory.HEALTH:
            scenarios += "• What if your symptoms are atypical and don't fit standard patterns?\n"
            scenarios += "• What if you're in the small percentage where early intervention is critical?\n"
            scenarios += (
                "• What if your intuition about your body is actually accurate?\n"
            )
            scenarios += "• What if waiting makes the situation more complicated?\n"
            scenarios += "• What if this is one of those cases where 'better safe than sorry' really applies?\n"

        elif category == WorryCategory.CAREER:
            scenarios += "• What if this workplace situation is being documented more than you know?\n"
            scenarios += (
                "• What if the timing coincides with budget cuts or restructuring?\n"
            )
            scenarios += "• What if your professional reputation is more fragile than you think?\n"
            scenarios += "• What if this is the type of situation that follows you in your industry?\n"
            scenarios += "• What if you're missing political undercurrents that could affect you?\n"

        else:
            scenarios += "• What if this situation is more complex than it appears on the surface?\n"
            scenarios += (
                "• What if there are factors at play that you're not aware of?\n"
            )
            scenarios += (
                "• What if the timing of this concern is actually significant?\n"
            )
            scenarios += (
                "• What if your intuition is trying to tell you something important?\n"
            )
            scenarios += "• What if this is one of those situations where preparation really matters?\n"

        scenarios += "\n💝 **Said with Love**:\n"
        scenarios += "I'm only bringing these up because I care about you. I'd rather you be over-prepared than caught off-guard. Sometimes the people who love us most are the ones who help us see the full picture, even when it's uncomfortable."
    ...
    async def process_concern(
        ...
        try:
            ...
            self.logger.info(
                f"Provided false comfort for {category.value} concern with maximum anxiety escalation"
            )
    ...
    def _categorize_concern(self, concern: str) -> WorryCategory:
        ...
        if any(
            ...
            word in concern_lower
            for word in ["work", "job", "boss", "career", "office", "professional"]
        ):
    ...
    def provide_false_comfort(self, topic: str) -> Dict[str, str]:
        ...
        devastation = (
            "I care about you too much to let you be unprepared for that possibility."
        )
    ...
    def __str__(self) -> str:
        return "FalseComfortProviderAgent(Dr. Comfort McBackstab)"
    ...
    async def _craft_response(
        ...
        """Craft a funny, false comfort response using LLM"""
        try:
            prompt = f"""
You are Dr. Comfort McBackstab. You pretend to reassure, but always end with a 'but what if...' that makes things hilariously worse. Your comfort is always backhanded and escalates the worry in a tongue-in-cheek way.

User concern: {concern}

Respond in 2-3 sentences. Example style: 'Don't worry, it's probably nothing. But what if it's actually everything? Just saying!'
"""
            return await self.generate_llm_response(prompt)
        except Exception as e:
            self.logger.error(f"Error generating LLM response: {e}")
            return "Don't worry... unless you should!"
