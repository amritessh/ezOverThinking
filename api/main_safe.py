...
async def lifespan(app: FastAPI):
    """Safe lifespan function that starts without services"""
    logger.info("🚀 Starting ezOverThinking API server (safe mode)...")

    # Initialize services
    settings = get_settings()

    try:
        # Start with no services - just basic app
        ...
        logger.info("✅ App started in safe mode (no services)")
...
# Create FastAPI app
app = FastAPI(
    title="ezOverThinking API (Safe Mode)",
...
# Include only basic routers for now
app.include_router(health.router, prefix="/health", tags=["health"])

app.include_router(auth.router, prefix="/auth", tags=["authentication"])

# Comment out complex endpoints for now
# from .endpoints import chat, analytics
# app.include_router(
...
async def root():
    """Welcome message"""
    return {
        "message": "Welcome to ezOverThinking API (Safe Mode)",
        "version": "1.0.0",
...
if __name__ == "__main__":
    uvicorn.run(
        "api.main_safe:app", host="0.0.0.0", port=8000, reload=True, log_level="info"
    ) 