...
from src.models.schemas import (
    ChatRequest, ChatResponse, UserConcern, ConversationState,
    AgentResponse, AnxietyLevel, ConversationAnalytics
)
...
async def send_message(
    ...
    orchestrator: ConversationOrchestrator = Depends(),
    anxiety_tracker: AnxietyTracker = Depends(),
...
async def continue_conversation(
    ...
    try:
        ...
        if not conversation_state or not conversation_state.last_concern:
            raise HTTPException(
                status_code=400,
                detail="No active conversation to continue"
            )
        
        # Continue with last concern
        result = await orchestrator.process_conversation_turn(
            user_id=user_id,
            user_concern=conversation_state.last_concern
        )
...
async def get_conversation_state(
    ...
    try:
        ...
        if not state:
            raise HTTPException(
                status_code=404,
                detail="No conversation state found"
            )
