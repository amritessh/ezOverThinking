...
class ConnectionManager:
    ...
    async def broadcast_to_user(self, user_id: str, message: WebSocketMessage):
        ...
        connections_to_send = [
            conn_id for conn_id, uid in self.user_sessions.items() if uid == user_id
        ]
    ...
    async def send_anxiety_update(
        self, connection_id: str, anxiety_level: AnxietyLevel
    ):
...
class WebSocketHandler:
    ...
    def __init__(
        self, orchestrator: ConversationOrchestrator, anxiety_tracker: AnxietyTracker
    ):
    ...
    async def _handle_user_concern(
        self, connection_id: str, user_id: str, content: str
    ):
        """Handle user concern input"""
        try:
            # Create user concern
            user_concern = UserConcern(
                user_id=user_id, original_worry=content, timestamp=datetime.now()
            )
    ...
    async def _process_conversation_turn(
        self, connection_id: str, user_id: str, user_concern: UserConcern
    ):
        ...
        try:
            ...
            agent_response = await self.orchestrator.orchestrate_response(
                conversation_id, user_concern.original_worry
            )

            # Create a result object similar to what was expected
            result = type(
                "Result", (), {"responses": [agent_response], "should_continue": True}
            )()
            ...
            if conversation_state:
                current_anxiety = await self.anxiety_tracker.get_real_time_anxiety(
                    conversation_state.conversation_id
                )
                await self.connection_manager.send_anxiety_update(
                    connection_id, current_anxiety
                )
    ...
    async def _handle_continue_conversation(self, connection_id: str, user_id: str):
        ...
        try:
            ...
            await self._process_conversation_turn(
                connection_id, user_id, conversation_state.last_concern
            )
...
def initialize_websocket_handler(
    orchestrator: ConversationOrchestrator, anxiety_tracker: AnxietyTracker
):
